...
const ModalViewComment = ({ comment, showModal, setShowModal }) => {
                ...
                <div class="relative p-2 w-full max-w-md h-full md:h-auto">
                    <div class="border-2 relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="p-4 text-center">
                            ...
                            {comment ?
                                <div>
                                    <div className='flex gap-2 items-center justify-center'>
                                        <BsCheck />
                                        <span className='font-bold text-neutral-700'> Kỹ năng nghe: </span>
                                        <span>{comment.listen}</span>
                                    </div>
                                    <div className='flex gap-2 items-center justify-center'>
                                        <BsCheck />
                                        <span className='font-bold text-neutral-700'> Kỹ năng từ vựng: </span>
                                        <span>{comment.vocabulary}</span>
                                    </div>
                                    <div className='flex gap-2 items-center justify-center'>
                                        <BsCheck />
                                        <span className='font-bold text-neutral-700' > Kỹ năng nói: </span>
                                        <span>{comment.read}</span>
                                    </div>
                                    <div>
                                        <div className='flex gap-2 items-center justify-center mt-4'>
                                            <GoPrimitiveDot />
                                            <div className='font-bold text-neutral-700'> Nhận xét của gia sư</div>
                                        </div>
                            ...
                            <button data-modal-toggle="popup-modal" type="button" class="mt-4 text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-400 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600" onClick={() => setShowModal(false)}>Đóng</button>
                        </div>
                    </div>
                </div>
            </div>}
